...
def smooth(growth_curves_filtered, growth_curves, loess_frac):
    growth_curves_filtered_smoothed = smooth_growth_curves(growth_curves_filtered, loess_frac)
    unfiltered_growth_curves_smoothed = smooth_growth_curves(growth_curves, loess_frac)
    return growth_curves_filtered_smoothed, unfiltered_growth_curves_smoothed

#@task
def filter_curves(layout, growth_curves, group_col, threshold):
...
def run_growth_processing(EXPERIMENT_DIR, met_bool):
    ...
    filtered_growth_curves, outlier_info = filter_curves(layout, raw_growth_curves, 'summary', threshold)
    outlier_info.to_csv(EXPERIMENT_DIR / 'results/growth/outliers.tsv', sep = '\t')
