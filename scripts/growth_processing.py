...
EXPERIMENT_DIR = Path(
    '/Users/danbru/Library/CloudStorage/OneDrive-Chalmers/Desktop/GenExp/experiments/good caffeine/arginine_202503131539')
...
def save_plots(EXPERIMENT_DIR, growth_curves_filtered_smoothed, raw_growth_curves, violinplot_df, mu_per_well, layout):
    ...
    plot_group_averages_in_hours(
        growth_curves_filtered_smoothed, group_col="Summary")
    # save the figure to file
    plt.savefig(EXPERIMENT_DIR / 'results/plots/averaged_growth_curves.png')
    plt.close('all')    # close the figure window

    # Plot a grid of them? along with the windows extracted for the growth rate calculation
    plot_growth_curves(raw_growth_curves, annotations_df=mu_per_well)
    # save the figure to file
    plt.savefig(EXPERIMENT_DIR / 'results/plots/growth_curves.png')
    plt.close('all')    # close the figure window
    ...
    # save the figure to file
    plt.savefig(EXPERIMENT_DIR / 'results/plots/growth_metrics.png')
    plt.close('all')    # close the figure window
...
def smooth(growth_curves_filtered, growth_curves, window_size):
    growth_curves_filtered_smoothed = smooth_growth_curves(
        growth_curves_filtered, window=window_size, method="mirror")
    ...
    unfiltered_growth_curves_smoothed = smooth_growth_curves(
        growth_curves, window=window_size, method="mirror")
...
def filter_curves(layout, growth_curves, group_col, method, threshold=3):
    # Filter for outliers curves using MAD (default),
    growth_curves_filtered = filter_outlier_growth_curves(
        layout, growth_curves, group_col="Summary", method=method, threshold=threshold)
...
def run_growth_processing(EXPERIMENT_DIR, testing_bool):
    ...
    filtered_growth_curves = filter_curves(
        layout, raw_growth_curves, 'summary', outlier_method, threshold)

    # growth_curves = blank_processing(layout,raw_growth_curves, n, fillin_value, config.BLANK_SUBTRACTION)
    filtered_blanked_growth_curves = blank_processing(
        layout, filtered_growth_curves, n, fillin_value, config.BLANK_SUBTRACTION)
    ...
    growth_curves_filtered_smoothed, unfiltered_growth_curves_smoothed = smooth(
        filtered_blanked_growth_curves, filtered_growth_curves, window_size)
