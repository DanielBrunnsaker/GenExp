...
def hypothesis_generation(output_folder,  N):
    ...
    if int(N) > 1:
        ...
        for nr_hyp in range(1,N+1):
            ...
            hyp_text = open(EXPERIMENT_DIR / f"hypothesis/generated_hypotheses/initial_stage/hypothesis_{nr_hyp}.txt", "r").read()
            complete_prompt += hyp_text +'\n\n'    
        
        completion = prompt_gpt_for_hypothesis(EXPERIMENT_DIR / '../../context/hypothesis_selection.txt', complete_prompt, key, config.LLM_PATTERN_SELECTION_TEMPERATURE, config.LLM_PATTERN_SELECTION_MODEL)
        ...
        selected_hypothesis = open(EXPERIMENT_DIR / f"hypothesis/generated_hypotheses/initial_stage/hypothesis_{N_extracted}.txt", "r").read()
        hypothesis_details = load_json(EXPERIMENT_DIR / f"hypothesis/generated_hypotheses/initial_stage/hypothesis_details_{N_extracted}.json")
        ...
        selected_hypothesis = open(EXPERIMENT_DIR / f"hypothesis/generated_hypotheses/initial_stage/hypothesis_{N}.txt", "r").read()
        hypothesis_details = load_json(EXPERIMENT_DIR / f"hypothesis/generated_hypotheses/initial_stage/hypothesis_details_{N}.json")
    ...
    N_extracted = int(re.search(r"HYPOTHESIS NUMBER #(\d+)", final_selection_text).group(1))
    finalized_hypothesis = open(EXPERIMENT_DIR / f"hypothesis/generated_hypotheses/second_stage/hypothesis_{N_extracted}.txt", "r").read()
