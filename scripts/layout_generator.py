...
def visualize_grid(plate_layout):
    ...
    grid = pd.DataFrame(np.nan, index=list("ABCDEFGH"), columns=[
                        f"{i:02d}" for i in range(1, 13)])

    # Map each unique value of CONCuM to a unique combination of S and T, handle "Media" as "Blank"
    unique_values = {val: ("Blank" if val == "Media" else f"S{idx // 2}T{idx % 2}")
                     for idx, val in enumerate(plate_layout['CONCuM'].unique())}
...
def plate_filler(plate_layout):
    ...
    all_wells = [f'{row}{col}' for row, col in itertools.product(
        ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'], [f'{i:02d}' for i in range(1, 13)])]
    # Find missing wells
    ...
    df_filled['well'] = pd.Categorical(
        df_filled['well'], categories=all_wells, ordered=True)
...
def generate_layout(output_path):
    ...
    experiment_entry = [entry for entry in json_data['experiments']
                        if 'type' in entry and 'experiment' in entry['type'].lower()][0]
