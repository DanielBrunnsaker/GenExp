...
def pattern_selection(target, alpha, N, output_folder):
    ...
    # Load reference data
    aaSet = pd.read_excel(BASE_DIR / '../data/AA.xls', sheet_name='intracellular_concentration_mM').set_index('ORF').iloc[:,1:]
    
    # Path for coefficients (update based on latest results)
    coefficients_path = (Path(BASE_DIR) / Path('../results/coefficients/20241114')).resolve()
    hypotheses, all_hypotheses = load_and_filter_hypotheses(coefficients_path, target, aaSet, BASE_DIR)

    # Filter predictable amino acids
    metrics = pd.read_csv(BASE_DIR / '../results/metrics/eCV_results_20241114.csv').groupby('Amino acid').mean()
