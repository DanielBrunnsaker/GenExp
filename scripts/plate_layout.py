...
def generate_layout(output_folder):
    ...
    reference_layout = load_json(
        EXPERIMENT_DIR / '../../plaid/reference_plate.json')
    ...
    experiment_entry = [entry for entry in json_data['experiments']
                        if 'type' in entry and 'negative' in entry['type'].lower()][0]
    negcontrol_name = experiment_entry['media_supplementation'].replace(
        ",", "_")

    experiment_entry = [entry for entry in json_data['experiments']
                        if 'type' in entry and 'experiment' in entry['type'].lower()][0]
    treatment_name = experiment_entry['treatment'].replace(",", "_")
    supplementation_name = experiment_entry['media_supplementation'].replace(
        ",", "_")
